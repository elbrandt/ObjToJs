...
def calc_vertex_normals(obj):
    ...
    obj['fvn'] = []
    # for each triangle:
    # 1. calculate the normal, 2. enter it into the vn list, 3. add it's index to the fvn list
...
def write_js(file, obj, model_name, stats):
    ...
    with open(file, 'w') as f:
        ...
        # write list of vertex normals (if we have them)
        if None not in obj['fvn']:
            write_point_list(f, obj, 'vn', 'vertex normals',
                             f'{model_name}.vertexNormals')
        else:
            print('\nNOTE: Skipping writing vertex normals to .js because they were not present (or incomplete) in .obj file.')
